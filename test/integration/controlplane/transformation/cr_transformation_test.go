...
// permitUserToDoVerbOnCR is a parameterized version of permitUserToDoVerbOnSecret
// that works with any API group and resource.
func permitUserToDoVerbOnCR(t *testing.T, client *clientset.Clientset, user, namespace string, verbs []string, apiGroup, resource string) {
	t.Helper()
...
func createFooCR(t *testing.T, dynamicClient dynamic.Interface, name, namespace string) *unstructured.Unstructured {
	...
	created, err := dynamicClient.Resource(fooGVR).Namespace(namespace).Create(context.TODO(), obj, metav1.CreateOptions{})
	if err != nil {
...
func TestCRAllowUnsafeMalformedObjectDeletionFeature(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(fmt.Sprintf("%s/%t", string(genericfeatures.AllowUnsafeMalformedObjectDeletion), tc.featureEnabled), func(t *testing.T) {
			...
			test, err := newTransformTest(t, transformTestConfig{transformerConfigYAML: crAESGCMConfigYAML, reload: true})
			if err != nil {
			...
			test.runResource(test.TContext, unSealWithGCMTransformer, aesGCMPrefix, "cr.bar.com", "v1", "foos", fooObj.GetName(), fooObj.GetNamespace())
...
func TestCRListCorruptObjects(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(fmt.Sprintf("%s/%t", string(genericfeatures.AllowUnsafeMalformedObjectDeletion), tc.featureEnabled), func(t *testing.T) {
			...
			test, err := newTransformTest(t, transformTestConfig{transformerConfigYAML: crAESGCMConfigYAML, reload: true, storageConfig: storageConfig})
			if err != nil {
				t.Fatalf("failed to setup test, error was %v", err)
			}
			defer test.cleanUp()
